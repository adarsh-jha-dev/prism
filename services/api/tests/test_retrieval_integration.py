...
async def test_the_nearest_chunks_come_back_first(collection_id: UUID) -> None:
    await seed(
        collection_id,
...
async def test_the_score_is_similarity_not_distance(collection_id: UUID) -> None:
    ...
    ((hit,)) = await search_chunks(
        "anything", collection_id=collection_id, provider=QueryProvider(), settings=settings()
    )
    assert hit.score == pytest.approx(0.9, abs=1e-5)


async def test_k_bounds_the_number_of_hits(collection_id: UUID) -> None:
    await seed(collection_id, [(f"chunk {n}", graded(0.9 - n / 100)) for n in range(20)])

    hits = await search_chunks(
        "anything", collection_id=collection_id, k=3, provider=QueryProvider(), settings=settings()
    )
    assert len(hits) == 3
    assert [hit.content for hit in hits] == ["chunk 0", "chunk 1", "chunk 2"]


async def test_k_defaults_to_the_configured_top_k(collection_id: UUID) -> None:
...
async def test_a_better_match_in_another_collection_is_never_returned(
    collection_id: UUID, other_collection_id: UUID
) -> None:
...
async def test_k_is_still_filled_when_another_collection_dominates_the_table(
    ...
    hits = await search_chunks(
        "anything", collection_id=collection_id, k=10, provider=QueryProvider(), settings=settings()
    )
...
async def test_an_unknown_collection_is_refused() -> None:
    ...
    with pytest.raises(CollectionNotFoundError, match=f"collection {missing} does not exist"):
        await search_chunks(
            "anything", collection_id=missing, provider=QueryProvider(), settings=settings()
        )
...
async def test_a_mismatched_provider_is_refused_before_the_query_is_embedded(
    ...
    with pytest.raises(EmbeddingModelMismatchError):
        await search_chunks(
            "anything", collection_id=collection_id, provider=provider, settings=settings()
        )
    assert provider.queries == []


async def test_a_hit_carries_what_a_citation_needs(collection_id: UUID) -> None:
...
async def test_an_empty_query_is_refused_without_a_database_round_trip() -> None:
    with pytest.raises(ValueError, match="query is empty"):
        await search_chunks(
            "   ", collection_id=uuid7(), provider=QueryProvider(), settings=settings()
        )
